...
if __name__ == '__main__':
    # 指定图片和XML文件的存储路径
    img_path = r"../ccpd_data"  # 设置源图片的路径。这里使用的是相对路径，指向当前脚本所在目录的上级目录下的ccpd_data文件夹。
    save_path = r"D:\lg\BaiduSyncdisk\project\person_code\project_self\chepai_OCR\11"  # 设置XML文件的保存路径。这是一个绝对路径，指向一个具体的目录。
